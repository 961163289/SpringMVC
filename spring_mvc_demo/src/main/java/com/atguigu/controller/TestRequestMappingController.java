 ...
 *      "!param"：表示当前所匹配的请求参数中一定不能携带param参数
 *      "param=value"：表示当前所匹配的请求参数中必须携带param参数且值必须为value
 *      "param!=value"：表示当前所匹配的请求参数中可以不携带param参数，若携带值一定不能是value
 *
 *  若当前请求满足@RequestMapping注解的value和method属性，但是不满足params属性，
 *  此时页面回报错400：Parameter conditions "username, password!=123456" not met for actual request parameters:
 *                  username={admin}, password={123456}
 *
 * 5、@RequestMapping注解的headers属性
 * 作用：通过请求的请求头信息匹配请求，即浏览器发送的请求的请求头信息必须满足headers属性的设置
 *
 * 若当前请求满足@RequestMapping注解的value和method属性，但是请求头信息不匹配
 * 此时页面报错：404 - 资源未找到
 *
